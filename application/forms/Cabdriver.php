...
class Application_Form_Cabdriver extends Zend_Form
{
    // Метод init() вызовется по умолчанию
    public function init()
    {
        // Задаём имя форме
        $this->setName('Cabdriver');
        ...
        */
        ...
        $namber_car = new Zend_Form_Element_Text('namber_car');
        $namber_car->setLabel('Номер машины')
            ->setRequired(true)
            ->addFilter('StripTags')
            ->addFilter('StringTrim')
            ->addValidator('NotEmpty', true,
        ...
        // Создаём элемент hidden c именем = status
        $status = new Zend_Form_Element_Hidden('status');
        $status->addFilter('StripTags');        
        ...
        $this->addElements(array($id, $name, $contract, $phone, $namber_car, $list_car, $status, $submit));
    }
